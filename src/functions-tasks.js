...
function getArgumentsCount(funcs) {
  const result = [];
  funcs.forEach((el) => result.push(el.length));
  return result;
}
...
function getPolynom(...rest) {
  function polynom(x) {
    ...
    for (let i = 0; i < rest.length; i += 1) {
      if (!(rest[i] === 0)) {
        if (i < rest.length - 1) result += rest[i] * x ** (rest.length - i - 1);
        else result += rest[i];
      }
    }
    return result;
...
function memoize(func) {
  ...
  return function result() {
    if (res === null) res = func.call(null);
    return res;
  };
}
...
function logger(func, logFunc) {
  return function result(...rest) {
    ...
    rest.forEach((el, index) => {
      args += JSON.stringify(el);
      if (index < rest.length - 1) args += ',';
    });
    res += `${func.name}(${args}) `;
    logFunc.call(null, `${res}starts`);
    const funcResult = func.call(null, ...rest);
    logFunc.call(null, `${res}ends`);
    return funcResult;
  };
}
...
function partialUsingArguments(fn, ...rest) {
  ...
  function result(...args) {
    if (args.length === 0) return fn(...initial);
    return fn(...initial, ...args);
  }
  return result;
}
